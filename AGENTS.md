...
**The Orchestrator is the sole execution controller for DAM. Every DAM application invocation enters through and remains under its control**, whether initiated by CLI or a future interface. Continuous and direct/single-operation execution are modes/scopes of that one architecture, never separate execution paths. The normal entry point starts a continuous orchestrated session; explicit commands such as `dam scan` and `dam teach list` enter the same Orchestrator in direct mode. The Orchestrator determines mode and requested scope, invokes the appropriate function/service, receives its result, and decides whether the run continues, redirects, resumes, or stops. A direct invocation stops after its requested operation completes. CLI commands must never call application operations through a bypass path, and functions must not inspect original CLI arguments to decide continuation. The same service contract and business behavior apply in either mode. The Orchestrator owns session lifecycle, workflow/path state, current and completed stages as appropriate, permitted transitions, interruption, continuation or stop, and future resume. It invokes only implemented and permitted stages. **Services/functions know how to perform DAM operations; the Orchestrator knows when and why they run.** The Orchestrator does not own Scan, Classification, Teaching, Rules, or Action business logic.
...
## Future temporary-evidence presentation lifetime

When temporary sensitive evidence is displayed, the presentation layer must actively remove it from surfaces DAM controls when the interaction ends, where supported. Terminal implementations must use the strongest supported screen/scrollback-clearing mechanism; `clear` or `cls` alone does not guarantee scrollback erasure. GUI implementations must discard the temporary view and its retained presentation state. DAM must release its own references and prohibit persistence, replay, and history where required, but must not claim secure erasure of externally controlled terminal scrollback, shell redirection, external terminal logging, screen recordings, or OS/process memory copies outside its practical control. This is a future requirement; temporary evidence retrieval and presentation clearing are not implemented.
