...
Steps 14B and 14C are complete, and Step 14C live acceptance passed. Architecture review is complete; preparatory operation/presentation migrations now cover `teach list`, `teach show`, `teach status`, `teach preview`, and `teach save`. Public `teach resume` still uses the older boundary.
...
## Complete: Teaching save boundary

**COMPLETE — commit subject `Add DAM teaching save boundary` (this change; 777 tests passed, including 33 new focused tests).** `dam teach save` converts CLI arguments into immutable `SaveTeaching` input containing only the work/category, confirmation fingerprint, optional ITEM, and supported file overrides. `save_teaching` owns its Storage lifetime and invokes the existing `TeachingService.confirm` once. Teaching still owns fingerprint/scope validation, durable intent before YAML mutation, learned-rule save/reconciliation, configuration reload/provenance, independent exact-item reevaluation, immutable EVAL predecessor chains, queue transitions, and recoverable TEACH checkpoints. No Teaching, learned-rule persistence, storage/schema, or Gmail implementation changed.

The control contract reuses `QueryControlResult`/`QueryDisposition` through the minimal `TeachingSaveControlResult` extension. `INCOMPLETE` records a returned unfinished Teaching outcome; it requests no next Operation. A separate boolean `presentation_failed` preserves the known completed/incomplete mutation disposition if projection or rendering fails. No source, category, rule, fingerprint, configuration, evaluation, repository row, or display payload enters control. Known exceptions return a sanitized failure (`REJECTED`); unexpected exceptions return `FAILED`. Neither means rollback or proves that no writes occurred: an exception can happen after durable intent, YAML save, partial EVAL commits, or completion. Existing durable status/list/resume capabilities remain the means of inspecting/recovering such work; the boundary does not infer a recovery state or retry.

`TeachingSaveDisplay` contains only the existing displayed TEACH ID/state and reevaluated/resolved/unresolved/insufficient counts. The existing sink delivers it after storage closes to the stateless renderer. No configuration fingerprint is added to save output because it was not previously displayed. Successful and pending output/exit behavior remains compatible. Classification-learning-only wording, `authority=false`, `executable=false`, and zero Gmail actions remain explicit. Rendering never validates confirmation scope. Presentation failure leaves committed work intact, performs no automatic second confirmation, and produces a fixed diagnostic directing the user to inspect status before retrying.

Failure-injection tests cover before intent, intent before YAML, YAML before its next checkpoint, configuration reload, partial reevaluation, durable completion before reporting, and post-commit projection/rendering failure or interruption. Existing public `teach resume` recovers the supported checkpoints without duplicate EVALs/rules; repeated completed confirmations preserve history. Tests also cover exact output, stale/changed confirmation scope, permanent category identity, no authority grants, source exclusion, immutable exact-item history/provenance, aggregate-only disclosure, and cross-operation payload lifetime.

Public `teach resume` remains on its older presentation boundary; its recovery implementation and the rest of Teaching are unchanged. This does not complete the Teaching migration or introduce an Orchestrator, Session IDs, continuous mode, Rules Queue, action execution, temporary evidence, or terminal clearing. `Storage.open` retains its existing setup/migration behavior.
